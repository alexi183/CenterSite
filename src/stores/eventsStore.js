...
export class eventsStore {
    ...
    @action likes = (id) => {
        axios.get(`/api/v1/likes/event/${id}/${this.cookie.value}`)
            .then(response => {
    ...
    categoryEventHandler = async () => {
        ...
        console.log(data)
        return (
            this.eventCategoryList = data.items,
    ...
    @action getCalendarEvents = (month, year) => {
        axios.get(`/api/v1/calendar-events/?month=${month}&year=${year}`).then(response => {
            runInAction(() => {
    ...
    @action getDisabledData = (month, year) => {
        axios.get(`/api/v1/calendar-dates/?month=${month}&year=${year}`).then(response => {
    ...
    @action onDatePickerMonth = (date) => {
        ...
        axios.get(`/api/v1/calendar-events/?month=${this.parsedMonth}&year=${this.parsedYear}`).then(response => {
    ...
    @action onDatePickerDateSelected = date => {
        ...
        axios.get(`/api/v1/calendar-events/?day=${this.parsedDay}&month=${this.parsedMonth}&year=${this.parsedYear}`).then(response => {
            this.calendarEvents = response.data;
