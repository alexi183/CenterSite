...
// import {CalendarIcon} from "../Elements/Icons/Icons";
...
@inject("eventsStore", "educateEventsStore")
@observer
class EducateEvents extends Component {

    async componentDidMount() {
        await this.props.eventsStore.getDate()

        await this.props.educateEventsStore.getEduEvents(
            await this.props.educateEventsStore.monthNumber
        )
    }
  ...
  render() {
    ...
    if (loading) {
      return <div style={{display: 'flex', alignItems: 'center', height: '500px'}}><Spinner /></div>
    }
        ...
        <div className="edu-events__slider-content">
          {EduEvents[0] ? (
            EduEvents.map((item, i) => (
              <Link
                to={`event-calendar/${item.id}`}
                className="edu-events__slider-row
                                                edu-events__link"
                key={i}
              >
                <Fragment>
                  {item.date_start === item.date_end ? (
                    <div className="edu-events__date edu-events__date_blue">
                      ...
                      <div className="edu-events__month">
                        {parsedMonthString(
                          moment(item.date_start).format("MMMM")
                        )}
                      </div>
                    </div>
                  ) : (
                    <div
                      className="edu-events__date edu-events__date_blue"
                      style={{
                        height: "auto",
                        paddingTop: "4.5px",
                        paddingBottom: "4.5px"
                    ...
                    >
                      <div>
                        {/*                 <span className="edu-events__day" style={{
                                                                               fontSize: "23px"
                                                                           }}>
            ...
            <div>В данном месяце событий нет</div>
          )}
        </div>
      );
    }
      ...
      <section className="edu-events">
        <div className="section-wrapper">
          ...
          <div className="edu-events-wrapper">
            {monthNumber !== 12 ? (
              <div className="edu-events__col" style={{paddingLeft: '0'}}>
                <Link
                  to={`event-calendar/${sortNearestEventList[0] && sortNearestEventList[0].id}`}
                  className="edu-events__cell edu-events__background-img"
                  style={{
                    background: `linear-gradient(to top, #000000 0%, rgba(0, 0, 0, 0.33) 67%, rgba(255, 255, 255, 0) 100%),
                                  url(${
                                    sortNearestEventList[0] &&
                                    sortNearestEventList[0].all_thumbnails
                                      ? sortNearestEventList[0].all_thumbnails.previewPicture_c_eduevent686x364
                                      : dataImgNo
                                  })`
                  }}
                >
                  {sortNearestEventList[0] && (
                    <div className="edu-events__inner">
                      <div className="edu-events__date-row">
                        {sortNearestEventList[0].date_start === sortNearestEventList[0].date_end ? (
                          <div className="edu-events__date edu-events__date_red">
                            <span className="edu-events__day">
                              {sortNearestEventList[0] &&
                                moment(sortNearestEventList[0].date_start).format("D")}
                            </span>
                            <div className="edu-events__month">
                              {parsedMonthString(
                                sortNearestEventList[0] &&
                                  moment(sortNearestEventList[0].date_start).format("MMMM")
                              )}
                            </div>
                          </div>
                        ) : (
                          <div
                            className="edu-events__date edu-events__date_red"
                            style={{
                              height: "auto",
                              paddingTop: "4.5px",
                              paddingBottom: "4.5px"
                          ...
                          >
                            <div>
                              ...
                              <div className="edu-events__day-month">
                                {sortNearestEventList[0] &&
                                  moment(sortNearestEventList[0].date_start).format(
                                    "D MMMM"
                                  )}
                              </div>
                            </div>
                            ...
                            <div>
                              {/*   <span className="edu-events__day">
                                            {EduEvents[0] && moment(EduEvents[0].date_end).format('D')}
                                        </span>*/}
                              <div className="edu-events__day-month">
                                {sortNearestEventList[0] &&
                                  moment(sortNearestEventList[0].date_end).format(
                                    "D MMMM"
                                  )}
                              </div>
                            </div>
                        ...
                        <div className="edu-events__title ml-3">
                          <Truncate lines={2}>
                            {sortNearestEventList[0] && sortNearestEventList[0].title}
                          </Truncate>
                        </div>
                      </div>
                    </div>
              ...
              <div className="edu-events__col">
                <Link
                  to={`event-calendar/${EduEvents[5] && EduEvents[5].id}`}
                  className="edu-events__cell edu-events__background-img"
                  style={{
                    background: `linear-gradient(to top, #000000 0%, rgba(0, 0, 0, 0.33) 67%, rgba(255, 255, 255, 0) 100%),
          url(${
            EduEvents[5] &&
            EduEvents[5].thumb_urls &&
            EduEvents[5].thumb_urls.previewPicture
              ? EduEvents[5].thumb_urls.previewPicture.original
              : dataImgNo
          })`
                  }}
                >
                  {EduEvents[5] && (
                    <div className="edu-events__inner">
                      <div className="edu-events__date-row">
                        {/* {EduEvents[5].date_start === EduEvents[0].date_end ? ( */}
                          <div className="edu-events__date edu-events__date_red">
                            <span className="edu-events__day">
                              {EduEvents[5] &&
                                moment(EduEvents[5].date_start).format("D")}
                            </span>
                            <div className="edu-events__month">
                              {parsedMonthString(
                                EduEvents[5] &&
                                  moment(EduEvents[5].date_start).format("MMMM")
                              )}
                            </div>
                          </div>
                        <div className="edu-events__title ml-3">
                          <Truncate lines={2}>
                            {EduEvents[5] && EduEvents[5].title}
                          </Truncate>
                        </div>
                      </div>
                    </div>
            ...
            <div className="edu-events__col" style={{paddingRight: '0'}}>
              <div className="edu-events__slider">
                <div className="edu-events__slider-head">
                  <div
                    className="edu-events__slider-prev"
                    onClick={monthDec}
                  >
                    <ArrowType3 />
                  </div>
                  ...
                  <div
                    className="edu-events__slider-next"
                    onClick={monthInc}
                  >
                    <ArrowType3 />
                  </div>
